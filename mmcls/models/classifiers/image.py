...
@CLASSIFIERS.register_module()
class SiameseClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 init_cfg=None):
        super(SiameseClassifier, self).__init__(init_cfg)
    ...
    def forward_train(self, img1, img2, gt_label, **kwargs):
        """Forward computation during training.

        Args:
            img1 (Tensor): of shape (N, C, H, W) encoding input images.
                Typically these should be mean centered and std scaled.
            img2 (Tensor): of shape (N, C, H, W) encoding input images.
                Typically these should be mean centered and std scaled.
            gt_label (Tensor): It should be of shape (N, 1) encoding the
                ground-truth label of input images for single label task. It
                shoulf be of shape (N, C) encoding the ground-truth label
        ...
        losses = dict()
        loss = self.head.forward_train(x1, x2, gt_label)
        losses.update(loss)

        return losses

    def forward_test(self, img1, img2, **kwargs):
        """
        Args:
        ...
        """
        return self.simple_test(img1, img2, **kwargs)

    @auto_fp16(apply_to=('img1', 'img2'))
    def forward(self, img1, img2, return_loss=True, **kwargs):
        ...
        """
        if return_loss:
            return self.forward_train(img1, img2, **kwargs)
        else:
            return self.forward_test(img1, img2, **kwargs)
    ...
    def simple_test(self, img1, img2, img_metas):
        """Test without augmentation."""
        x1 = self.extract_feat(img1)
        x2 = self.extract_feat(img2)
        x_dims = len(x1.shape)
